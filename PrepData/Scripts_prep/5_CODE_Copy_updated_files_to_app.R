...
#pImage <- "C:/ALL_PROJECTS/PastFromSpace_DRAFT4/PrepData/PGR/y2021/PGR20210810 10 August 2021.tif"
#fImage <- "C:/ALL_PROJECTS/PastFromSpace_DRAFT4/PrepData/FOO/y2021/FOO20210810 10 August 2021.tif"
lf <- list.files("C:/ALL_PROJECTS/PastFromSpace_DRAFT4/PrepData/PGR/y2021", pattern=".tif$", full.names=TRUE)
pImage <- lf[length(lf)]

lf2 <- list.files("C:/ALL_PROJECTS/PastFromSpace_DRAFT4/PrepData/FOO/y2021", pattern=".tif$", full.names=TRUE)
...
writeDir <- "C:/ALL_PROJECTS/PastFromSpace_DRAFT4/ShinyPastures/data"

#data being copied and where its coming from: 

#Dirs with new data to copy:
inDir <- "C:/ALL_PROJECTS/PastFromSpace_DRAFT4/PrepData/PGR_cumulative"
inFoo <- "C:/ALL_PROJECTS/PastFromSpace_DRAFT4/PrepData/FOO"
...
# first copy old one into "drafts  use overwrite=TRUE
#file.copy(paste0(writeDir, "/", p2, ".rds"), paste0(writeDir, "/PreviousDraft"), copy.date=TRUE, overwrite=TRUE)
#file.copy(paste0(writeDir, "/", f2, ".rds"), paste0(writeDir, "/PreviousDraft"), copy.date=TRUE, overwrite=TRUE)

 #copy new files in
#file.copy(paste0(inDir,  "/", p2, ".rds"), writeDir, copy.date=TRUE, overwrite=TRUE)
#file.copy(paste0(inFoo,  "/", f2, ".rds"), writeDir, copy.date=TRUE, overwrite=TRUE)
